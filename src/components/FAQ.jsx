...
const FAQ = () => {
  ...
  return (
    <section className="py-16 md:py-24 px-4 md:px-6 bg-[#F4F2F1] font-sans">
      <div className="max-w-6xl mx-auto">
        
        {/* --- Header --- */}
        <div className="text-center mb-12 md:mb-16">
          <div className="inline-block px-4 py-1.5 mb-4 md:mb-6 rounded-full bg-white border border-gray-200">
            <span className="text-[10px] md:text-xs font-bold text-gray-900 uppercase tracking-wide">FAQs</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 lg:gap-x-16 gap-y-4 mb-12 md:mb-20 items-start">
          {faqData.map((faq, idx) => (
            <div key={idx} className="border-b border-gray-200 h-full">
              <div 
                onClick={() => toggleFaq(idx)}
                className="flex items-start justify-between py-5 md:py-6 cursor-pointer group hover:bg-black/5 transition-colors px-2 rounded-lg"
              >
                <span className="text-sm md:text-base font-semibold text-gray-900 pr-4">{faq.question}</span>
                <svg 
                  className={`w-5 h-5 text-gray-900 flex-shrink-0 mt-0.5 transition-transform duration-300 ${activeIndex === idx ? 'rotate-45' : ''}`} 
                  fill="none" 
                  viewBox="0 0 24 24" 
                  stroke="currentColor" 
                  strokeWidth="2"
        ...
        <div className="bg-white rounded-2xl p-6 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm border border-gray-100">
          ...
          <button className="w-full md:w-auto bg-[#111] text-white text-sm font-bold py-3.5 px-8 rounded-lg hover:bg-black transition-all hover:-translate-y-0.5 whitespace-nowrap active:scale-95">
            Buy Now
          </button>
        </div>
